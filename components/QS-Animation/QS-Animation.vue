<template>
	<view class="QS-Animation" ref="icon" :animation="Animation" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend"
	 @toucherror="touchend" @tap="_onClick">
		<slot></slot>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			shook(type) {
				...
				this.Animation = {};
				const preAnimationFn = () => {
					this.Ani.rotateY(-25).translateX('-10%').scaleY(.7).step({
						duration: AddDur.addDuration(50)
					});
				}
