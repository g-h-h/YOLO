<template>
	<view class="uni-card uni-border" :class="{ 'uni-card--full': isFull === true || isFull === 'true', 'uni-card--shadow': isShadow === true || isShadow === 'true'}">
		...
		<view  v-if="note" class="uni-card__footer uni-border-top">
			<slot name="footer">
				<text class="uni-card__footer-text">	
				</text>
				<button class="mini-btn" type="default" size="mini" @click="remote_1">删除</button>
				<button class="mini-btn" type="default" size="mini"  @click="edit">编辑</button>
			</slot>
		</view>
	</view>
</template>
